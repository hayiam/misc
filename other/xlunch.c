...
void handleKeyPress(XEvent ev) {
            ...
            || keycode==XK_d || keycode==XK_u || keycode==XK_Home || keycode==XK_End)
    {
        ...
        if (hovered_entry <= columns/3)
            f = (entries_count - columns) / 3;
            else if (hovered_entry <= 2*columns/3+1)
                f = 2*((entries_count - columns) / 3);
                else
                    f = 3*((entries_count - columns) / 3);
        ...
        if (entries_count % columns !=0 && hovered_entry < entries_count - entries_count % columns)
            movecount = i;
        else
            movecount = entries_count % columns; 
...
void renderEntry(Imlib_Image buffer, char title[256], node_t * current, Cursor * c, int up_x, int up_y) {
    ...
    if (current->hovered)
    {
        if (hoverset == MOUSE) *c = XCreateFontCursor(disp,XC_hand2);
    ...
    for (;;)
    {
        ...
        if(poll_result < 0){
            ...
            for (current_update = updates;   current_update;    current_update = imlib_updates_get_next(current_update))
            {
                ...
                button_t *button = buttons;
                while (button != NULL) {
                    if(button->hovered) c = XCreateFontCursor(disp,XC_hand2);
