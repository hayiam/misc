...
enum { SchemeNorm, SchemeSel, SchemeStatus, SchemeButton, SchemeTagsSel, SchemeTagsNorm, SchemeInfoSel, SchemeInfoNorm }; /* color schemes */
...
static void views(const Arg *arg);
...
/* uncomment to inable focus follows mouse */
...
// added "drawbar: Don't expend effort drawing bar if it is occluded" patch from dwm 6.3 (right two lines below)
    ...
    /* if topbar use gap for it for floating windows (vanilla: selmon->showbar ? bh : 0 */
    /* starty = selmon->topbar && selmon->showbar ? bh : 0; */
    /* starty = 0 - no gap for panel while doing moveresize */
        starty = 0;
...
//		    togglescratch(&(Arg){.v = scratchpadvifm});
//		}
// end of added lines
	if (selmon->sel)
...
{
	int ocx, ocy, nw, nh, ocx2, ocy2, nx, ny;
	Client *c;
	Monitor *m;
	XEvent ev;
